...
class GitMoApp:
    ...
    def _ribbon_button(
        ...
        *,
        ...
        underline = tk.Frame(item, bg=THEME["accent"] if active else THEME["panel"], height=3)
        underline.pack(side="bottom", fill="x", pady=(10, 0))
        ...
        def on_enter(_event=None) -> None:
            ...
            if not active:
                underline.configure(bg=THEME["button_hover"])
    ...
    def show_repo_selection_screen(self) -> None:
        ...
        search = search_card.inner  # type: ignore[attr-defined]
        self._card_label(search, "Search", bold=True, side="left", padx=(0, 10))
        search_entry = ttk.Entry(search, textvariable=search_var, width=34)
    ...
    def _status_text_and_color(self, state: str, detail: str) -> tuple[str, str]:
        if state == "idle":
            color = THEME["success"]
            label = "Watching"
        elif state in {"synced", "pulled", "pending"}:
            color = THEME["sync"]
            label = "Syncing" if state == "pending" else state.title()
        ...
        return (f"● {label}: {detail}" if detail else f"● {label}", color)
